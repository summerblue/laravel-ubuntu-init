...
sed -i "s/listen =.*/listen = 127.1.0.1:9000/" /etc/php/7.1/fpm/pool.d/www.conf
...
mysql --user="root" --password="${MYSQL_ROOT_PASSWORD}" -e "GRANT ALL ON *.* TO '${MYSQL_NORMAL_USER}'@'127.1.0.1' IDENTIFIED BY '${MYSQL_NORMAL_USER_PASSWORD}' WITH GRANT OPTION;"
